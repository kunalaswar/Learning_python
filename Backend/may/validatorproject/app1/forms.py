...
def course_validator(c):
    if c not in ["python", "java", "mysql"]:
        raise forms.ValidationError("Course Must Be python, java, mysql")
    else:
        return c
...
class EmailForm(forms.Form): 
    ...
    def clean_fee(self): #* this function define in class so this is not reusable and this is checking only fee means one field
        d = super().clean()
